...
{
  return memcached_failed(rc) && rc != MEMCACHED_CREATED_STORED && rc != MEMCACHED_ELEMENT_EXISTS && rc != MEMCACHED_NO_SERVERS && rc != MEMCACHED_MAXIMUM_RETURN;
}
  ...
  for (my = 0; my < LOOP; my++)
  {
    for (userid=0; userid<NUM_OF_USERS; userid++) {
      ...
      if (SAMPLE_PIPE) {
        ...
        // eflags
        uint32_t eflag = 1001;
        const unsigned char **eflags = (const unsigned char **)malloc(sizeof(unsigned char *) * NUM_OF_PIPED_ITEMS);
        size_t eflags_length[NUM_OF_PIPED_ITEMS];

        // values
        uint32_t value = 5000;
        const char **values = (const char **)malloc(sizeof(char *) * NUM_OF_PIPED_ITEMS);
